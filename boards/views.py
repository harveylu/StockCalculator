    ...
    if request.method == 'POST':
        try:
            ...
            if "error" in json_result:
                ...
                print type(json_result['totalInvestment'])
                print json_result['totalInvestment']
                print str(round(json_result['totalInvestment'], 3))
                json_result['totalInvestment'] = "{0:,.2f}".format(json_result['totalInvestment'])
                json_result['actualInvestment'] = "{0:,.2f}".format(json_result['actualInvestment'])
                json_result['remainingBalance'] = "{0:,.2f}".format(json_result['remainingBalance'])
                print type(json_result['totalInvestment'])
                print json_result['totalInvestment']
            ...
            msg = e
            return render(request, 'report-error.html', {'result': msg})
